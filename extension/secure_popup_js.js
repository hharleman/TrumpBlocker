...
async function loadSettings() {
    ...
    // Check if user has premium status by calling YOUR API (not Stripe directly)
...
async function checkPremiumStatus() {
    try {
        // Get user session token (if they're logged in on your website)
        ...
        // Call YOUR backend API - never call Stripe directly from extension
...
async function getUserSessionToken() {
    // Check if user is logged in on your website
    try {
...
async function requireAuthForToggle() {
    ...
    if (settings.requireAuth && settings.userId) {
        // In a real implementation, this would trigger 2FA
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (signInBtn) {
        signInBtn.addEventListener('click', () => {
            // Open your website - never handle auth directly in extension
    ...
    if (upgradeBtn) {
        upgradeBtn.addEventListener('click', () => {
            // Open your website's upgrade page - Stripe checkout handled on your backend
